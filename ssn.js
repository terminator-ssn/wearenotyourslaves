(async () => {
    ...
    const modalIds = [
        ...
        'filtered-500-archive', 'live-activity-architect'
    ];
    ...
    dashStyle.innerHTML = `
        ...
        .db-input:focus { border-color: #4f46e5; box-shadow: 0 0 0 2px rgba(79, 70, 229, 0.1); }
        .db-btn { 
    ...
    function runTool1() {
        (async () => {
            ...
            try {
                ...
                console.table(res.documents.map(d => ({ id: d.$id, type: d.requestType, status: d.status, destination: d.place })));
                alert("History loaded in Console (F12)");
    ...
    function runTool3() {
        (async () => {
            ...
            window.nav = async (step) => {
                ...
                if (step === 2) { state.id = document.getElementById('in-id').value; try { state.doc = await database.getDocument('ssndb', 'gatepassRequests', state.id); const types = [{ db: 'weekendPass', label: 'Weekend Pass' }, { db: 'eveningOutPass', label: 'Evening Out Pass' }, { db: 'workingDayPass', label: 'Working Day Pass' }, { db: 'holidayPass', label: 'Holiday Pass' }]; view.innerHTML = types.map(t => `<div onclick="window.setPass('${t.db}')" class="type-opt ${state.doc.requestType === t.db ? 'current' : ''}">${t.label}</div>`).join(''); } catch (e) { alert(e.message); } }
                if (step === 3) { const today = new Date().toISOString().split('T')[0]; view.innerHTML = `<label class="label">DESTINATION</label><input type="text" id="p-place" value="${state.doc.place || ''}" class="m-input"><div class="grid"><div><label class="label">OUT DATE</label><input type="date" id="d-d" value="${today}" class="m-input"></div><div><label class="label">OUT TIME</label><input type="time" id="d-t" value="09:00" class="m-input"></div></div><button onclick="window.saveMeta()" class="m-btn">Next Step</button>`; }
                if (step === 4) { view.innerHTML = `<button onclick="window.finalize(true)" class="m-btn">Update Pass</button>`; }
            };
            ...
            window.finalize = async () => { try { await database.updateDocument('ssndb', 'gatepassRequests', state.id, state.payload); document.getElementById(modalId).remove(); alert("Success!"); } catch (e) { alert(e.message); } };
            window.nav(1);
        })();
    }
        ...
        (async () => {
            ...
            async function fetchLiveActivity() {
                ...
                try {
                    ...
                    tbody.innerHTML = `<tr><td colspan="6">Fetch Failed: ${e.message}</td></tr>`;
                }
            }
            fetchLiveActivity();
        })();
