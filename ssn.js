(async () => {
    ...
    // --- SHARED CSS (Preserving Original Styles) ---
    ...
    dashStyle.innerHTML = `
        ...
        /* Results Table Styling (Matches Tool 8/9) */
        .v-card-full { background: #fff; width: 98%; max-width: 1350px; height: 85vh; border-radius: 8px; display: flex; flex-direction: column; overflow: hidden; box-shadow: 0 10px 30px rgba(0,0,0,0.5); font-family: sans-serif;}
    ...
    // ==========================================
    // 1. History Fetcher (NEW UI INTEGRATED)
    // ==========================================
    async function runHistoryFetcher() {
        ...
        document.body.insertAdjacentHTML('beforeend', `
            <div id="${modalId}" style="z-index: 2147483647; position: fixed; inset: 0; display: flex; align-items: center; justify-content: center; background: rgba(0, 0, 0, 0.7);">
                <div class="v-card-full">
                    <div style="padding: 20px; border-bottom: 1px solid #ddd; background: #f8f9fa; display: flex; justify-content: space-between; align-items: center;">
                        <div>
                            <strong id="h-title" style="font-size:16px;">Fetching Student History...</strong>
                        </div>
                    ...
                    <div style="flex: 1; overflow: auto;">
                        <table class="v-table">
                            <thead>
                                <tr>
                                    <th>REQUEST ID</th><th>TYPE</th><th>STATUS</th><th>TRAVEL (OUT / IN)</th><th>ACTUAL SCANS</th><th>PLACE</th>
                                </tr>
                            </thead>
        ...
        try {
            ...
            if (res.documents.length === 0) {
                tbody.innerHTML = `<tr><td colspan="6" style="text-align:center; padding:50px;">No records found for this ID.</td></tr>`;
                return;
            }

            const student = res.documents[0].students;
            document.getElementById('h-title').innerText = `Gatepass History: ${student?.aadharName || 'Unknown'} (${userId})`;
            ...
            res.documents.forEach(d => {
                tbody.insertAdjacentHTML('beforeend', `
                    <tr>
                        ...
                        <td><span style="padding:4px 8px; border-radius:4px; font-size:10px; font-weight:bold; background:#e0f2fe; color:#0369a1;">${d.status.toUpperCase()}</span></td>
                        <td style="font-size:10px;">
                            Out: ${new Date(d.startDate).toLocaleString('en-IN')}<br>
                            In: ${new Date(d.endDate).toLocaleString('en-IN')}
                        </td>
                        <td style="font-size:10px;">
                            Out: ${d.outTime ? new Date(d.outTime).toLocaleString('en-IN') : '---'}<br>
                            In: ${d.inTime ? new Date(d.inTime).toLocaleString('en-IN') : '---'}
                        </td>
    ...
    function runTool2() {
        (async () => {
            ...
            window.move = (step) => {
                ...
                if (step === 2) { state.userId = document.getElementById('in-uid').value; const types = [{ db: 'weekendPass', label: '1. Weekend Pass' }, { db: 'eveningOutPass', label: '2. Evening Out Pass' }, { db: 'workingDayPass', label: '3. Working Day Pass' }, { db: 'holidayPass', label: '4. Holiday Pass' }]; view.innerHTML = `<label class="label">SELECT PASS CATEGORY</label><div style="margin-top:10px;">` + types.map(t => `<button onclick="window.setT('${t.db}')" class="type-btn">${t.label}</button>`).join('') + `</div>`; }
    ...
    // ==========================================
    // 3 - 9. Administrative Tools (Remaining Untouched)
    // ==========================================
    ...
    function runTool4() {
        (async () => {
            ...
            const database = new Appwrite.Databases(client);
            let activeId = '';
            window.verifyPass = async () => { activeId = document.getElementById('target-id').value; try { const d = await database.getDocument('ssndb', 'gatepassRequests', activeId); const today = new Date().toISOString().split('T')[0]; document.getElementById('s-content').innerHTML = `<div class="info-box"><label class="label">Applied Pass Type</label><span class="val">${d.requestType}</span><div class="grid"><div><label class="label">Requested Out</label><span class="val" style="font-size:11px;">${new Date(d.startDate).toLocaleString('en-IN')}</span></div><div><label class="label">Requested In</label><span class="val" style="font-size:11px;">${new Date(d.endDate).toLocaleString('en-IN')}</span></div></div></div><label class="label">Actual Departure</label><div class="grid"><input type="date" id="act-d" value="${today}" class="s-input"><input type="time" id="act-t" value="16:00" class="s-input"></div><button onclick="window.recordExit()" class="s-btn">Record Departure</button><button onclick="window.location.reload()" class="s-btn-alt">Back</button>`; } catch (e) { alert("ID not found."); } };
    ...
    function runTool9() {
        (async () => {
            ...
            async function fetchLiveActivity() {
                ...
                try {
                    const res = await database.listDocuments('ssndb', 'gatepassRequests', [Appwrite.Query.orderDesc('$updatedAt'), Appwrite.Query.limit(500)]);
                    res.documents.forEach(d => {
                        const updated = new Date(d.$updatedAt); const created = new Date(d.$createdAt); let statusHTML = '';
                        if (d.inTime) statusHTML = `<span class="status-tag s-back">Returned</span>`; else if (d.outTime) statusHTML = `<span class="status-tag s-out">Scanned Out</span>`; else if (d.wardenApproval === 'approved') statusHTML = `<span class="status-tag s-app">Approved</span>`; else if (d.status === 'rejected') statusHTML = `<span class="status-tag s-rej">Rejected</span>`; else { let pWho = "Warden"; if (d.mentorApproval === 'pending') pWho = "Mentor"; else if (d.supervisorApproval === 'pending') pWho = "Supervisor"; const isNew = Math.abs(updated - created) < 5000; statusHTML = isNew ? `<span class="status-tag s-new">New</span>` : `<span class="status-tag s-wait">Pending ${pWho}</span>`; }
                        tbody.insertAdjacentHTML('beforeend', `<tr><td><span class="full-id">${d.$id}</span></td><td style="font-weight:bold;">${d.students?.aadharName || 'N/A'}</td><td>${d.students?.department || 'N/A'}</td><td>${d.requestType}</td><td style="font-size:10px;">Out: ${new Date(d.startDate).toLocaleString()}<br>In: ${new Date(d.endDate).toLocaleString()}</td><td>${statusHTML}<span class="time-hint" style="color:#6366f1; font-weight:bold;">Last Change: ${updated.toLocaleTimeString()}</span></td></tr>`);
                    });
                    document.getElementById('v-status').innerHTML = `<span style="color:#16a34a;">✅ Active</span>`;
