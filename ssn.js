(async () => {
    ...
    function runTool10() {
        (async () => {
            ...
            style.innerHTML = `
                #${modalId} { z-index: 2147483647; position: fixed; inset: 0; display: flex; align-items: center; justify-content: center; background: rgba(0, 0, 0, 0.6); font-family: sans-serif; } 
                .p-card { background: #fff; width: 450px; border-radius: 8px; overflow: hidden; border: 1px solid #ddd; box-shadow: 0 10px 25px rgba(0,0,0,0.3); display: flex; flex-direction: column; max-height: 90vh; } 
                .p-header { background: #f8f9fa; padding: 15px; border-bottom: 1px solid #eee; text-align: center; font-weight: bold; } 
                .p-body { padding: 20px; overflow-y: auto; flex: 1; } 
                .p-input { width: 100%; padding: 8px 10px; margin-top: 5px; border: 1px solid #ccc; border-radius: 4px; box-sizing: border-box; font-size: 13px; outline: none; } 
                .p-btn { width: 100%; padding: 12px; margin-top: 15px; background: #0056b3; color: #fff; border: none; border-radius: 4px; cursor: pointer; font-weight: bold; } 
                .f-row { margin-bottom: 12px; padding-bottom: 8px; border-bottom: 1px solid #f9f9f9; } 
                .f-label { font-size: 10px; font-weight: bold; color: #777; text-transform: uppercase; } 
                .f-val { font-size: 14px; color: #333; font-weight: 600; margin-top: 2px; }
            `;
            document.head.appendChild(style);
            
            const modalHTML = `<div id="${modalId}"><div class="p-card"><div class="p-header">Profile Master Architect</div><div id="p-content" class="p-body"><label class="f-label">USER ID</label><input type="text" id="target-uid" class="p-input" onkeyup="if(event.key==='Enter') window.fetchP()"><button onclick="window.fetchP()" class="p-btn">Fetch Profile</button></div><div id="p-foot" style="padding: 0 20px 20px; display:none;"><button id="p-toggle" onclick="window.toggleP()" class="p-btn">Enable Edit Mode</button><button onclick="document.getElementById('${modalId}').remove(); window.returnToSsnHome();" style="border:none; background:none; color:#999; width:100%; margin-top:10px; cursor:pointer; font-size:11px;">CANCEL</button></div></div></div>`;
            document.body.insertAdjacentHTML('beforeend', modalHTML);
            ...
            window.renderP = () => { let h = ''; Object.keys(sch).forEach(k => { let v = curD[k] || '---'; if (k === '$createdAt') v = new Date(v).toLocaleString('en-IN'); h += `<div class="f-row"><label class="f-label">${sch[k]}</label>${editM && k !== '$createdAt' ? `<input type="text" id="i-${k}" class="p-input" value="${curD[k] || ''}">` : `<div class="f-val">${v}</div>`}</div>`; }); document.getElementById('p-content').innerHTML = h; };
