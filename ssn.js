(async () => {
    ...
    dashStyle.innerHTML = `
        ...
        /* Tool 10 Specific styles matching original minimalist style */
    ...
    <div id="${masterContainerId}">
        <div class="db-card">
            ...
            <div id="db-grid-sec" class="db-grid">
                ...
                <button class="db-btn" onclick="window.launchSsnTool(9)">9. Live Activity Monitor</button>
                <button class="db-btn" onclick="window.launchSsnTool(10)">10. Profile Master</button>
    ...
    // Tools 1-9 (Verbatim as per your requirement)
    async function runTool1() {
        ...
        try {
            ...
            window.hDel = async (id) => { if(!confirm("Permanently delete this request?")) return; try { await database.deleteDocument('ssndb', 'gatepassRequests', id); runTool1(); } catch(e) { alert(e.message); } };
    ...
    function runTool3(directId = null) {
        (async () => {
            ...
            window.nav = async (step, forcedId = null) => {
                ...
                if (step === 2) { state.id = forcedId || document.getElementById('in-id').value; if (!state.id) return; try { state.doc = await database.getDocument('ssndb', 'gatepassRequests', state.id); const types = [{ db: 'weekendPass', label: 'Weekend Pass' }, { db: 'eveningOutPass', label: 'Evening Out Pass' }, { db: 'workingDayPass', label: 'Working Day Pass' }, { db: 'holidayPass', label: 'Holiday Pass' }]; view.innerHTML = `<label class="label">SELECT PASS TYPE</label><div style="margin-top:10px;">` + types.map(t => `<div onclick="window.setPass('${t.db}')" class="type-opt ${state.doc.requestType === t.db ? 'current' : ''}">${t.label} ${state.doc.requestType === t.db ? '<span>(Current)</span>' : ''}</div>`).join('') + `</div>`; } catch (e) { alert("Error: " + e.message); window.nav(1); } }
            ...
            window.finalize = async (scan) => { if (scan) { state.payload.outTime = new Date(`${document.getElementById('s-d').value}T${document.getElementById('s-t').value}:00+05:30`).toISOString(); state.payload.inTime = null; } else { state.payload.outTime = null; state.payload.inTime = null; } try { await database.updateDocument('ssndb', 'gatepassRequests', state.id, state.payload); document.getElementById(modalId).remove(); alert("Pass updated successfully."); } catch (e) { alert("Failed: " + e.message); window.nav(1); } };
    ...
    function runTool9() {
        (async () => {
            ...
            document.head.appendChild(style);
            const modalHTML = `<div id="${modalId}"><div class="v-card"><div class="v-header"><div><strong style="font-size:16px;">Live Activity Monitor</strong><div id="v-status-live" style="font-size:11px; color:#64748b;">Monitoring system-wide database changes...</div></div><button onclick="document.getElementById('${modalId}').remove(); window.returnToSsnHome();" style="padding:6px 12px; cursor:pointer; background:#fff; border:1px solid #ccc; border-radius:4px; font-weight:bold;">Close</button></div><div class="v-body"><table class="v-table"><thead><tr><th>REQUEST ID</th><th>STUDENT NAME</th><th>DEPARTMENT</th><th>TYPE</th><th>TRAVEL DATES</th><th>STATUS</th></tr></thead><tbody id="v-tbody-live"></tbody></table></div></div></div>`;
            document.body.insertAdjacentHTML('beforeend', modalHTML);
    ...
    function runTool10() {
        (async () => {
            ...
            document.head.appendChild(style);
            const modalHTML = `<div id="${modalId}"><div class="pm-card"><div class="pm-header"><strong>Profile Master Architect</strong><button onclick="document.getElementById('${modalId}').remove(); window.returnToSsnHome();" style="background:none; border:none; color:#94a3b8; cursor:pointer; font-weight:bold;">✕</button></div><div id="pm-content" class="pm-body"><div style="text-align:center; padding:40px 0;"><p style="font-size:14px; color:#64748b; margin-bottom:20px;">Enter User ID to modify profile metadata.</p><input type="text" id="target-id-p" style="width:250px; padding:15px; border-radius:12px; border:2px solid #e2e8f0; text-align:center; font-family:monospace; font-size:20px; outline:none;" placeholder="2430xxx" onkeyup="if(event.key==='Enter') window.loadP()"><button onclick="window.loadP()" class="btn btn-edit" style="width:250px; display:block; margin: 15px auto;">INITIALIZE SESSION</button></div></div><div id="pm-footer" class="pm-footer" style="display:none;"><button id="btn-p-toggle" onclick="window.togglePMode()" class="btn btn-edit">ENABLE EDIT MODE</button><button onclick="window.location.reload()" class="btn btn-cancel">DISCONNECT</button></div></div></div>`;
            document.body.insertAdjacentHTML('beforeend', modalHTML);
            const gDB = database;
            let curD = null, isEM = false;
            const sch = { "$createdAt": "Created At (Read-only)", "aadharName": "Full Name", "gender": "Gender", "rollNo": "Roll Number", "degree": "Degree", "department": "Department", "year": "Year", "institute": "Institute", "emailId": "Email ID", "mentorEmailId": "Mentor Mail ID", "studentMobile": "Student Number", "fatherNumber": "Father Number", "motherNumber": "Mother Number", "guardianNumber": "Guardian Number", "hostel": "Hostel", "hostelBlock": "Hostel Block", "roomNo": "Room No", "roomType": "Room Type" };
            window.loadP = async () => { const id = document.getElementById('target-id-p').value; if (!id) return; try { curD = await gDB.getDocument('ssndb', 'students', id); window.renderP(); document.getElementById('pm-footer').style.display = 'flex'; } catch (e) { alert("Record not found."); } };
            window.renderP = () => { const body = document.getElementById('pm-content'); let html = ''; Object.keys(sch).forEach(key => { let val = curD[key] || '---'; if (key === '$createdAt') val = new Date(val).toLocaleString('en-IN'); html += `<div class="field-box"><label class="field-label">${sch[key]}</label>${isEM && key !== '$createdAt' ? `<input type="text" id="edit-${key}" class="field-input" value="${curD[key] || ''}">` : `<div class="field-val">${val}</div>`}</div>`; }); body.innerHTML = html; };
            window.togglePMode = () => { isEM = !isEM; const btn = document.getElementById('btn-p-toggle'); if (isEM) { btn.innerText = "SAVE CHANGES TO SERVER"; btn.className = "btn btn-save"; window.renderP(); } else { window.saveP(); } };
            window.saveP = async () => { let updatedData = {}; Object.keys(sch).forEach(key => { if (key !== '$createdAt') { const input = document.getElementById(`edit-${key}`); if (input) updatedData[key] = input.value; } }); try { curD = await gDB.updateDocument('ssndb', 'students', curD.$id, updatedData); isEM = false; document.getElementById('btn-p-toggle').innerText = "ENABLE EDIT MODE"; document.getElementById('btn-p-toggle').className = "btn btn-edit"; window.renderP(); alert("Database updated successfully."); } catch (e) { alert("Update Failed: " + e.message); isEM = true; window.renderP(); } };
        })();
    }
})();
